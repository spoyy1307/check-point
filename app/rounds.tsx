  ...
  return (
    ...
    >
      ...
      {PATROL_ROUNDS.map((round) => (
        <Pressable key={round.id} style={styles.card} onPress={() => router.push({ pathname: "/checkpoint", params: { round: round.id } })}>
          <View style={styles.main}>
            <Text style={styles.title}>{round.title}</Text>
            <Text style={styles.time}>{round.time}</Text>
          </View>
          <View style={styles.right}>
            <Text style={[styles.count, round.status === "late" && { color: COLORS.orange }]}>{round.completed}/{round.points} จุด</Text>
